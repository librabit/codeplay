...
def question10(woman): # woman 매개변수에 선택하는 인물 받음
    score = 0
    global score1 # 선배와의 연애점수
    score1 = 0
    global score2 # 동갑과의 연애점수
    score2 = 0
    global score3 # 후배와의 연애점수
    score3 = 0    
    for question in woman: # 각 대상별 지문 가져오기
        ...
        print(f"상대방 : {question[2][sel-1]}") # 선택한 결과 상대방 대사
...
while running: 
    ...
    if len(women) > 1:
        ...
        if chosen_woman == "선배":
            love_result = question10(questions_01)
            if love_result >= 12: # 연애성공점수         
                score1 = 1
            else:
                score1 = 0
            clear()
            ...
            love_result = question10(questions_02)
            if love_result >= 12: # 연애성공점수         
                score2 = 1
            else:
                score2 = 0
            clear()
            print(personal_ending1[score2])
            input("(다음으로 넘어가려면 엔터를 치세요...)")
        elif chosen_woman == "후배":
            love_result = question10(questions_03)
            if love_result >= 12: # 연애성공점수         
                score3 = 1
            else:
                score3 = 0
            clear()
            print(personal_ending1[score3])
            input("(다음으로 넘어가려면 엔터를 치세요...)")
        elif chosen_woman == "끝":
            running = False
